...
# creamos un mixim que es una clase que contiene una o varias definiciones y que se pueden eredar en otras clases (no tienen utilidad) 
class CenterWidgetMixim():
    def center(self):
        self.update()
        w = self.winfo_width()
        h = self.winfo_height()
...
class CreateClientWindow(Toplevel, CenterWidgetMixim):
    def __init__(self, parent):
        super().__init__(parent)
        self.title('Crear cliente')
        self.build()
        self.center()
        #Las siguientes dos funciones nos obligaran a realizar una funcion en la segunda ventana para luego poder pasar a la primera
    ...
    def build(self):
        ...
        # Creamos una lista de validaciones con la opcion de True, True, True o en este caso 0, 0, 0,
        self.validaciones = [0, 0, 0]

        #Luego portamos el boton de crear a nivel de instancia para poder utilizarlo
        self.crear = crear 
...
# Opcio mas corta usando validadores ternarios   
...
# La opcion que se muestra a continuacion, es la opcion mas largar en lineas de codigo
    def validate(self, event, index):
        valor = event.widget.get()
        if index ==0:
        ...
        # Cambiar el estado del boton CREAR con base en las validaciones realizadas Self, Event o index
...
#Creamos la clase de edicion para la opcion de modificacion "esta es igual a la anterior" 

class EditClientWindow(Toplevel, CenterWidgetMixim):
    def __init__(self, parent):
        super().__init__(parent)
    ...
    def edit_client(self):
        cliente = self.master.treeview.item(cliente, values=(self.dni.get(), self.nombre.get(), self.apellido.get()))
...
class MainWindow(Tk, CenterWidgetMixim):
    def __init__(self): 
        super().__init__()
    ...
    def build(self):
        ...
        treeview.heading('DNI', text='Nombre', anchor=CENTER)
        treeview.heading('Nombre', text='Nombre', anchor=CENTER)
        treeview.heading('Apellido', text='Nombre', anchor=CENTER)
...
# Creamos los botones de la parte inferior 
 ...
 # Creamos la subventana de borrar, con la funcion delete. 
